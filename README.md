...
| ジョブ | スクリプト | 実行頻度 | 概要 |
|---|---|---|---|
| Limitless同期 | `scripts/limitless-sync.ts` | 毎日 JST 01:00 | Limitless Pendant の前日データを db_limitless_raw に差分同期する |
| 日報生成 | `scripts/daily-lifelog.ts` | 毎日 JST 04:00 | Limitless の前日データから日報を生成し db_lifelog に書き込む |
| Identity抽出 | `scripts/weekly-identity.ts` | 毎週月曜 JST 05:00 | 前週の日報から価値観・意思決定を抽出し db_identity / db_decisions に反映する |
| データ整合性チェック | `scripts/data-integrity-check.ts` | 毎週月曜 JST 06:00 | limitless_raw と lifelog の整合性を検証しレポートを記録する |
...
| `LIMITLESS_API_TOKEN` | Limitless API キー（Limitless同期） |
